...
class LogicalOrOperator extends BinaryOperator {
  ...
  evalReturningRuntime(runtime: ValueRuntime): GetValueRuntimeResult {
    const [lhsExpr, rhsExpr] = this.args
    const allAssigns = Expressions.allNamesFrom(this.args)
    ...
    return lhsExpr
      ...
      .map(([value, nextRuntime]) => {
        const reconciledRuntime = Expressions.includeMissingNames(nextRuntime, allAssigns, lhsExpr)
...
export class NegateOperator extends UnaryOperator {
  ...
  operatorType(_runtime: TypeRuntime, lhs: Types.Type): GetTypeResult {
    if (lhs.isLiteral('float')) {

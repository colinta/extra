import {err, mapAll, ok, type Result, Guarantee} from '@extra-lang/result'
...
export function box(name: string, type: Type, identity: UniqueType = unique(name)) {
  return new BoxType(name, type, identity)
}
 ...
 */
export function enumType(definition: NamedEnumDefinitionType, member: EnumCase) {
  return new NamedEnumInstanceType(definition, member)
}
...
export function namedEnumDefinition({
  ...
  if (moreStatics) {
    for (const [name, type] of moreStatics(definition, enumInstance)) {
      definition.addStaticProp(name, type)
    }
  }
...
export function narrowNamedEnum(prevType: NamedEnumInstanceType, enumCase: EnumCase) {
  return new NamedEnumInstanceType(prevType.metaType, enumCase)
}
...
function combineTypes(lhs: Type, rhs: Type): Type {
  ...
  if (lhs instanceof BoxType) {
    return combineTypes(lhs.of, rhs)
  }

  if (rhs instanceof BoxType) {
...
export class GenericType extends Type {
  ...
  constructor(
    ...
    /**
     * Optional explicit type for this generic (e.g., from `Result<String>` where
     * T has resolvedType = String). Used by canBeAssignedTo to check assignability
     * against the concrete type, and by solveConstraints to validate hints.
     */
    public resolvedType?: Type | undefined,
    /**
     * Optional upper bound: `<T is Float>` constrains T to types assignable to Float.
     * After unification resolves T, the resolved type is checked against this bound.
     */
    readonly bound?: Type | undefined,
  ) {
    super()
  }
  ...
  toCode() {
    let code = this.name
    if (this.bound) {
      code += ` is ${this.bound.toCode()}`
    }
    if (this.resolvedType) {
      code += ` = ${this.resolvedType.toCode()}`
    }
    return code
  }
...
export class BoxType extends Type {
  ...
  declare static types: Record<string, ((box: BoxType) => Type) | undefined>
  ...
  toTruthyType() {
    ...
    return new BoxType(this.name, next, this.identity)
  }
  ...
  propAccessType(name: string | number) {
    ...
    return BoxType.types[name]?.(this)
  }
}
...
export class MetaFloatType extends NumberType<Narrowed.NarrowedFloat> {
  ...
  /**
   * A falsey float is the number 0
   */
  toFalseyType() {
    if (this.isOnlyTruthyType()) {
      return NeverType
...
export class ObjectType extends Type {
  ...
  /**
   * Instances of an object are always true – and shouldn't be used as a
   * conditional
   */
  isOnlyTruthyType() {
    return true
...
export abstract class EnumType extends Type {
  constructor(readonly member: EnumCase) {
    super()
  }
...
export class NamedEnumDefinitionType extends Type {
  ...
  instanceTypes: NamedEnumInstanceType[] = []
  ...
  get instanceType(): Type {
    if (this.instanceTypes.length === 1) {
      return this.instanceTypes[0]
    }
    return OneOfType.createOneOf(this.instanceTypes)
  }

  resolveInstanceTypes(instanceTypes: NamedEnumInstanceType[]) {
    this.instanceTypes = instanceTypes
  }

  lookupCase(name: string): NamedEnumInstanceType | undefined {
    return this.instanceTypes.find(t => t.member.name === name)
  }
  ...
  propAccessType(name: string | number): Type | undefined {
    ...
    const caseType = this.lookupCase(name)
    if (caseType) {
...
export class NamedEnumInstanceType extends EnumType {
   ...
   */
  toCode() {
    return `${this.metaType.name}.${this.member.name}`
  }
}
...
/**
 * Apply a substitution to a type, returning a new type with all generic
 * type variables replaced according to the mapping. Generics not present
 * in the substitution are left as-is (handles "foreign generics" from
 * outer scopes).
 *
...
export function applySubst(subst: Substitution, type: Type): Type {
  ...
  if (type instanceof GenericType) {
    return subst.get(type) ?? type
  }

  // No generics anywhere in this type — nothing to substitute
  if (!type.hasGeneric()) {
    return type
  }

  if (type instanceof ArrayType) {
    const inner = applySubst(subst, type.of)
    if (inner === type.of) return type
    return new ArrayType(inner, type.narrowedLength)
  }

  if (type instanceof DictType) {
    const inner = applySubst(subst, type.of)
    if (inner === type.of) return type
    return new DictType(inner, type.narrowedLength, type.narrowedNames)
  }

  if (type instanceof SetType) {
    const inner = applySubst(subst, type.of)
    if (inner === type.of) return type
    return new SetType(inner, type.narrowedLength)
  }
  ...
  if (type instanceof OneOfType) {
    ...
    for (const member of type.of) {
      const r = applySubst(subst, member)
      if (r !== member) changed = true
      resolved.push(r)
    }
    if (!changed) return type
    return OneOfType.createOneOf(resolved)
  }
  ...
  if (type instanceof ClassInstanceType) {
    const parent = type.parent ? applySubst(subst, type.parent) : undefined
    let changed = parent !== type.parent
    const props = new Map<string, Type>()
    for (const [key, propType] of type.myProps) {
      const r = applySubst(subst, propType)
  ...
  if (type instanceof FormulaType) {
    let changed = false
    const returnType = applySubst(subst, type.returnType)
    if (returnType !== type.returnType) changed = true

    const args = type.args.map(arg => {
      const r = applySubst(subst, arg.type)
...
function _applySubstToEnumCase<T extends Type>(
  subst: Substitution,
  member: EnumCase,
  original: T,
  create: (member: EnumCase) => T,
): T {
  let changed = false
  ...
  return create(new EnumCase(member.name, resolvedArgs))
}
...
export function narrowTypeIs(lhsType: Type, typeAssertion: Type): Type {
  ...
  if (lhsType instanceof EnumType && typeAssertion instanceof EnumType) {
    ...
    const lhsMember = lhsType.member
    const assertionMember = typeAssertion.member

    if (lhsMember.name !== assertionMember.name) {
      return NeverType
    }
    ...
    const resultCase = new EnumCase(lhsMember.name, newArgs)

    if (lhsType instanceof NamedEnumInstanceType) {
      return narrowNamedEnum(lhsType, resultCase)
    }

    if (lhsType instanceof AnonymousEnumType) {
      return narrowAnonymousEnum(lhsType, resultCase)
    }
...
export function narrowTypeIsNot(lhsType: Type, typeAssertion: Type): Type {
  ...
  if (lhsType instanceof EnumType && typeAssertion instanceof EnumType) {
    ...
    if (lhsMember.name !== assertionMember.name) {
      // different case names — lhs is not narrowed
      return lhsType
    }
    ...
    //     lhs: .a(Int)
    //     lhs !is .a
    //     --> NeverType (the assertion fully covers this case)
    ...
    if (narrowedCount === 0) {
      // all args fully covered — exclude this case
      return NeverType
    }
    ...
    // multiple args narrowed — can't narrow, keep original
    return lhsType
  }
...
export function compatibleWithBothTypes(lhs: Type, rhs: Type): Type {
  ...
  if (lhs instanceof BoxType && rhs instanceof BoxType) {
    if (lhs.identity !== rhs.identity) {
      return _privateOneOf([lhs, rhs])
    }

    const compatible = compatibleWithBothTypes(lhs.of, rhs.of)
    return new BoxType(lhs.name, compatible, lhs.identity)
  } else if (lhs instanceof BoxType || rhs instanceof BoxType) {
    return _privateOneOf([lhs, rhs])
  }
...
export function canBeAssignedTo(
  ...
  if (testType instanceof BoxType && assignTo instanceof BoxType) {
  ...
  } else if (testType instanceof BoxType || assignTo instanceof BoxType) {
    return why(false, `'${testType}' is not assignable to '${assignTo}'.`)
  }
  ...
  if (testType instanceof OneOfType) {
    ...
    if (nonGeneric.some(rhType => canBeAssignedTo(testType, rhType, reason))) {
      return true
    }

    return why(
      generic.some(rhType => canBeAssignedTo(testType, rhType, reason)),
  ...
  if (testType.isGeneric() && !assignTo.isGeneric()) {
    if (testType.resolvedType) {
      return canBeAssignedTo(testType.resolvedType, assignTo, reason)
    }

    // Unresolved generic — compatible with anything (constraints are
    // collected separately via generateConstraints)
    return true
  }

  if (assignTo.isGeneric()) {
    if (assignTo.resolvedType) {
      return canBeAssignedTo(testType, assignTo.resolvedType, reason)
    }

    // Unresolved generic — compatible with anything
    return true
  }
  ...
  if (testType.isLiteral() && assignTo.isLiteral()) {
    ...
    return canBeAssignedTo(testType.valueType(), assignTo, reason)
    ...
    for (const {lhs, rhs} of combined.value) {
      if (!canBeAssignedTo(lhs.type, rhs.type, reason)) {
        return why(
          false,
    ...
    const errorMessage = canBeAssignedToFormula(testType, assignTo)
...
function _checkFormulaArguments(
  ...
  // determines whether to support argument short-circuiting.
  ...
  for (const formulaArgument of sortedArgs) {
    ...
    } else if (!canBeAssignedTo(argumentType, formulaArgument.type, reason)) {
      if (formulaArgument.alias) {
        errors.push(
        ...
        namedFormula(
          ...
          [
            positionalArgument({
              name: 'apply',
              type: formula(
                [positionalArgument({name: 'input', type: box.of, isRequired: true})],
                T,
              ),
              isRequired: true,
            }),

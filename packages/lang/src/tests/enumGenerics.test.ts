  ...
  for (const caseType of enumDefType.instanceTypes) {
    ...
    if (member.args.length === 0) {
      ...
      runtimeTypes[`.${member.name}`] = [
        new Types.NamedFormulaType(member.name, caseType, argTypes, enumDefType.genericTypes),
        Values.nullValue(),
      ]
    }
  }
  return {enumDef, enumDefType}
}
...
`
  ...
  it('can use generic anonymous enums in function invocations', () => {
    // Defines a generic function with anonymous enum type containing generics.
    // The function compiles and the body's switch correctly narrows the enum cases.
    const code = `
    let
      fn map-it<T, U>(value: .nil | .val(T), mapper: fn(# input: T): U): U? =>
        switch value
        case .nil
          null
        case .val(v)
          mapper(v)
    in
      map-it(value: .val(5), mapper: fn(input: 4|5|6) => input + 1)
`
    const currentExpression = parse(code).get()
    const resolvedType = currentExpression.getType(typeRuntime).get()
...
`
  ...
  it('can use generic named enums in function invocations', () => {
    defineEnum(`\
enum Result<T> {
  .nil
  .val(T)
    ...
    const code = `
    let
      fn map-it<U, V>(value: Result(U), mapper: fn(# input: U): V): V? =>
        switch value
        case .nil
          null
        case .val(v)
...
enum Result<T> {
  .value(T)
    ...
    const resolvedType = currentExpression.getType(typeRuntime).get()
    expect(resolvedType).toEqual(
      Types.enumType(enumDefType, Types.enumCase('value', [Types.positionalProp(Types.int())])),
    )
  })
})
